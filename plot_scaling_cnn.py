...
if __name__ == '__main__':
    ...
    colorwheel = categorical_cmap(2, 5, cmap="tab10")
    color_range = 0 #0 = blue, 5 = orange, 10 = green
    colors = [colorwheel(color_range+i) for i in [4, 3, 2, 1, 0]]
    ...
    plt.plot(acc_flow_meas[layers == 3], delta_accs[layers == 3], color=colors[3], **line_kwargs)
    plt.plot(acc_flow_meas[layers == 4], delta_accs[layers == 4], color=colors[4], **line_kwargs)
    #plt.plot(acc_flow_meas[layers == 5], delta_accs[layers == 5], color=colors[2], **line_kwargs)
    ...
    plt.plot(x, slope * x + intercept, color='gainsboro')
    slope, intercept, rval1 = stats.linregress(acc_flow_meas[layers == 1], delta_accs[layers == 1])[:3]
    x = np.r_[min(acc_flow_meas), max(acc_flow_meas)]
    plt.plot(x, slope * x + intercept, color='silver')
    ...
    slope, intercept, rval3 = stats.linregress(acc_flow_meas[layers == 3], delta_accs[layers == 3])[:3]
    x = np.r_[min(acc_flow_meas), max(acc_flow_meas)]
    plt.plot(x, slope * x + intercept, color='dimgrey')
    slope, intercept, rval4 = stats.linregress(acc_flow_meas[layers == 4], delta_accs[layers == 4])[:3]
    x = np.r_[min(acc_flow_meas), max(acc_flow_meas)]
    plt.plot(x, slope * x + intercept, color='black')
    #slope, intercept, rval2 = stats.linregress(acc_flow_meas[layers == 5], delta_accs[layers == 5])[:3]
    #x = np.r_[min(acc_flow_meas), max(acc_flow_meas)]
    ...
    plt.text(0.05, 0.05, '$R^2_1 = %.2f$\n$R^2_2 = %.2f$\n$R^2_3 = %.2f$\n$R^2_4 = %.2f$\n$R^2_5 = %.2f$' % (rval0**2, rval1**2, rval2**2, rval3**2, rval4**2), fontsize=16,
             horizontalalignment='left', verticalalignment='bottom', transform=ax.transAxes)
    plt.tight_layout()

    color_range = 5 #0 = blue, 5 = orange, 10 = green
    colors = [colorwheel(color_range+i) for i in [4, 3, 2, 1, 0]]
    ...
    plt.plot(bias_flow_meas[layers == 3], delta_biases[layers == 3], color=colors[3], **line_kwargs)
    plt.plot(bias_flow_meas[layers == 4], delta_biases[layers == 4], color=colors[4], **line_kwargs)
    #plt.plot(bias_flow_meas[layers == 5], delta_biases[layers == 5], color=colors[2], **line_kwargs)
    # Compute regression lines for delta_acc & delta_bias vs. resp flow
    ...
    x = np.r_[min(bias_flow_meas), max(bias_flow_meas)]
    plt.plot(x, slope * x + intercept, color='darkgrey')
    slope, intercept, rval3 = stats.linregress(bias_flow_meas[layers == 3], delta_biases[layers == 3])[:3]
    x = np.r_[min(bias_flow_meas), max(bias_flow_meas)]
    plt.plot(x, slope * x + intercept, color='dimgrey')
    slope, intercept, rval4 = stats.linregress(bias_flow_meas[layers == 4], delta_biases[layers == 4])[:3]
    x = np.r_[min(bias_flow_meas), max(bias_flow_meas)]
    plt.plot(x, slope * x + intercept, color='black')
    #slope, intercept, rval5 = stats.linregress(bias_flow_meas[layers == 5], delta_biases[layers == 5])[:3]
    #x = np.r_[min(bias_flow_meas), max(bias_flow_meas)]
    #plt.plot(x, slope * x + intercept, color='k')
