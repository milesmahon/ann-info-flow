...
if __name__ == '__main__':
    ...
    plt.title('$\Delta_{acc}$ vs signed weighted acc flow', **title_kwargs)
    plt.xlabel('Signed weighted accuracy flow', **label_kwargs)
    plt.ylabel('Change in accuracy upon pruning', **label_kwargs)
    plt.tight_layout()

    plt.figure()
    plt.plot(bias_flows, delta_biases, 'C1o')
    plt.title('$\Delta_{bias}$ vs signed weighted bias flow', **title_kwargs)
    plt.xlabel('Signed weighted bias flow', **label_kwargs)
    plt.ylabel('Change in output bias upon pruning', **label_kwargs)
    plt.tight_layout()

    # Plot acc/bias change against absolute weighted information flows
    plt.figure()
    plt.plot(abs(acc_flows), delta_accs, 'C0o')
    plt.title('$\Delta_{acc}$ vs abs weighted acc flow', **title_kwargs)
    ...
    plt.title('$\Delta_{bias}$ vs abs weighted bias flow', **title_kwargs)
    ...
    plt.title('$\Delta_{acc}$ vs acc/bias flow ratio', **title_kwargs)
    ...
    plt.title('$\Delta_{bias}$ vs bias/acc flow ratio', **title_kwargs)
